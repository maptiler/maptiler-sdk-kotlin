...
class MTMapViewController(
    ...
    private var _navigationControlShowZoom: Boolean = true
    ...
    /**
     * navigationControl showZoom property adds zoom controls visibility.
     * Backed by navigationControl showZoom to toggle zoom controls visibility.
     */
    var navigationControlShowZoom: Boolean
        get() = _navigationControlShowZoom
        set(value) {
            _navigationControlShowZoom = value
            coroutineScope?.launch {
                bridge?.execute(
                    com.maptiler.maptilersdk.commands.misc.SetNavigationControlShowZoom(value),
                )
            }
        }
     ...
     */
    override suspend fun isMoving(): Boolean = navigableWorker.isMoving()
    ...
    override suspend fun pointDiv(
        ...
        scalar: Double,
    ): com.maptiler.maptilersdk.map.types.MTPoint = navigableWorker.pointDiv(point, scalar)

    override suspend fun pointDivByPoint(
        point1: com.maptiler.maptilersdk.map.types.MTPoint,
        point2: com.maptiler.maptilersdk.map.types.MTPoint,
    ): com.maptiler.maptilersdk.map.types.MTPoint = navigableWorker.pointDivByPoint(point1, point2)
    ...
    override suspend fun pointDist(
        point1: MTPoint,
        point2: MTPoint,
    ): Double = navigableWorker.pointDist(point1, point2)
    ...
    /**
     * Rotate this point around a pivot point by an angle.
     *
     * @param point The point to rotate.
     * @param angle The angle to rotate by, in radians.
