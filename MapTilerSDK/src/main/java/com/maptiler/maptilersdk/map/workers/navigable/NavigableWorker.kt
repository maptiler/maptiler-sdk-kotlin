...
import com.maptiler.maptilersdk.commands.misc.PointRound
import com.maptiler.maptilersdk.commands.misc.PointSub
import com.maptiler.maptilersdk.commands.misc.PointUnit
...
internal class NavigableWorker(
    ...
    override fun resetNorth(options: MTAnimationOptions?) {
        scope.launch {
            bridge.execute(
                ResetNorth(options),
            )
    ...
    override suspend fun isZooming(): Boolean {
        val returnTypeValue =
            bridge.execute(
                IsZooming(),
            )
    ...
    override suspend fun lngLatToArray(lngLat: LngLat): DoubleArray {
        val returnTypeValue =
            bridge.execute(
                LngLatToArray(lngLat),
            )

        return when (returnTypeValue) {
            is StringValue -> JsonConfig.json.decodeFromString<DoubleArray>(returnTypeValue.value)
            else -> DoubleArray(0)
        }
    }

    override suspend fun lngLatToString(lngLat: LngLat): String {
        val returnTypeValue =
            bridge.execute(
                LngLatToString(lngLat),
            )
    ...
    override suspend fun pointUnit(point: MTPoint): MTPoint {
        val returnTypeValue =
            bridge.execute(
                PointUnit(point),
            )
    ...
    override suspend fun pointRound(point: MTPoint): MTPoint {
        val returnTypeValue =
            bridge.execute(
                PointRound(point),
            )
